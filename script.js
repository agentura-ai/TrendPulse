...
async function checkLifetimeAccess(email) {
    ...
    try {
        const doc = await dbFirestore.collection('lifetimeUsers').doc(email.toLowerCase()).get();
...
async function grantLifetimeAccess(email) {
    if (!email) return;
    try {
        await dbFirestore.collection('lifetimeUsers').doc(email.toLowerCase()).set({
...
// Modify handleGoogleSignInCallback to use the new functions

...
function checkLifetimeAccess(email) {
    ...
    hasLifetimeAccess = lifetimeUsers.includes(normalizedEmail);
...
async function getTrends() {
    ...
    hasLifetimeAccess = checkLifetimeAccess(userEmail);

    // Check if trial is used and user doesn't have lifetime access
    if (trialUsed && !hasLifetimeAccess) {
        showMessage('Trial expired. Please contact us to get unlimited access.', 'info');
        setTimeout(() => {
            window.location.href = 'pricing.html';
        }, 2000);
        return;

...
const DB_NAME = 'TrendsPulseDB';
const DB_VERSION = 1;
const STORE_NAME = 'lifetimeUsers';
...
const LIFETIME_USERS_KEY = 'trendsPulseLifetimeUsers'; // New constant for storing lifetime access users

let trialUsed = localStorage.getItem(TRIAL_KEY) === 'true';
let userEmail = localStorage.getItem(USER_EMAIL_KEY);
let hasLifetimeAccess = false; // Will be set in checkLifetimeAccess()

// Get used trials and lifetime users from localStorage or initialize empty arrays
...
// Function to check if a user has lifetime access using IndexedDB
async function checkLifetimeAccess(email) {
    ...
    try {
        ...
        return new Promise((resolve) => {
            ...
            request.onsuccess = () => {
                hasLifetimeAccess = !!request.result;
                if (hasLifetimeAccess) {
                    localStorage.setItem(ACCESS_KEY, 'lifetime');
                }
                resolve(hasLifetimeAccess);
            };
...
// Function to grant lifetime access using IndexedDB
async function grantLifetimeAccess(email) {
    ...
    try {
        ...
        return new Promise((resolve, reject) => {
            ...
            const store = transaction.objectStore(STORE_NAME);
            const request = store.put({ email: email.toLowerCase(), timestamp: Date.now() });
...
// Modify handleGoogleSignInCallback to be async
async function handleGoogleSignInCallback(response) {
    try {
        ...
        // Check if user has lifetime access using IndexedDB
        const hasAccess = await checkLifetimeAccess(email);
        if (hasAccess) {
            showSuccessMessage('✅ Welcome back! Your lifetime access is active.');
            hideSignupModal();
...
// Modify getTrends function to use checkLifetimeAccess
async function getTrends() {
    ...
    hasLifetimeAccess = await checkLifetimeAccess(userEmail);

    // Check if trial is used and user doesn't have lifetime access
    if (trialUsed && !hasLifetimeAccess) {
        window.location.href = 'pricing.html';
        return;
    }
    ...
    try {
        ...
        if (newsData.items && newsData.items.length > 0) {
            // Only update trial status if user doesn't have lifetime access
            if (!hasLifetimeAccess) {
                // Add email to used trials if not already present
...
function updateUserEmailDisplay() {
    ...
    const hasLifetimeAccess = localStorage.getItem(ACCESS_KEY) === 'lifetime';
    const userEmailElement = document.getElementById('userEmail');
    
    if (userEmail) {
        userEmailElement.innerHTML = `
            <span title="Logged in email">👤 ${userEmail}</span>
            ${hasLifetimeAccess ? '<span class="lifetime-badge">✨ Lifetime Access</span>' : ''}
...
function handleLogout() {
    // Clear user session data but keep trials and lifetime users record
...
// Call updateUserEmailDisplay and check lifetime access when the page loads
document.addEventListener('DOMContentLoaded', () => {
    updateUserEmailDisplay();
    if (userEmail) {
        checkLifetimeAccess(userEmail);
    }
});

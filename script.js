...
async function getTrends() {
    ...
    try {
        ...
        if (newsData.items && newsData.items.length > 0) {
            // Add email to used trials if not already present and user doesn't have lifetime access
            if (!hasLifetimeAccess && !usedTrials.includes(userEmail.toLowerCase())) {
                usedTrials.push(userEmail.toLowerCase());
                localStorage.setItem(USED_TRIALS_KEY, JSON.stringify(usedTrials));
            }
            
            // Mark trial as used after successful search if user doesn't have lifetime access
...
function handleLogout() {
    ...
    localStorage.removeItem('trendsPulseAccess'); // Also clear lifetime access status
